 ...
 * @brief AsyncRTOS JSON-RPC server example 0
 * @version 0.9.0
 * @date 2023-04-17
 *
 * @copyright Copyright (c) 2023
 *
 * This example shows how to setup a JSON-RPC server over Websocket
 *
 * @note: The JSON-RPC server is implemented as a shared context rather than a
 ...
 * non-advisable and even troublesome in some occasions.
 *
 * For instance, in this example `aos_jrpc_server_call` is executed by the
 * Websocket task in `ws_on_data` using a generic future. If we had used an
 * awaitable and awaited it in-place, the Websocket task would be blocked and
 * incapable of responding to inputs until `jrpc_handler_dosomething` completed,
 * which could take a long time depending on what it does.
 *
 * Similar reasoning with a worse outcome applies to `aos_jrpc_server_call_cb`.
 * If we had used an awaitable there, the Websocket task would be blocked until
 ...
 *
 * Using generic futures in these cases ensures that the Websocket task is
 * always responsive regardless of how long handlers execution takes.
 *
 * Also note that the reasoning above doesn't apply to handlers, in which we are
 * free to use any kind of futures we want.
 */
#include <aos.h>
...
static aos_jrpc_server_t *_jrpc_server = NULL;
...
static void ws_on_data(const void *data, size_t data_len) {
  ...
  // Process input data through the JSON-RPC server
  // NOTE: Read the note at the top of the file on why we use a generic future
  aos_future_config_t config = {.cb = aos_jrpc_server_call_cb};
  aos_future_t *call_future =
      AOS_FUTURE_ALLOC_T(aos_jrpc_server_call)(&config, NULL, 0);
  aos_jrpc_server_call(_jrpc_server, data, call_future);
}
...
void app_main(void) {
  ...
  // Initialize AOS JSON-RPC server
  aos_jrpc_server_config_t server_config = {};
  _jrpc_server = aos_jrpc_server_alloc(&server_config);

  // Set JSON-RPC handlers
  aos_jrpc_server_handler_set(_jrpc_server, jrpc_handler_dosomething,
                              "dosomething");
  ...
  // All good! Server ready
}

static void aos_ws_client_send_text_cb(aos_future_t *future);
static void aos_jrpc_server_call_cb(aos_future_t *future) {
  // Save arguments and free resources
  AOS_ARGS_T(aos_jrpc_server_call) *args = aos_args_get(future);
  char *data = args->out_data;
  unsigned int error = args->out_err;
  aos_future_free(future);

  if (!error) {
    // Send response through Websocket
    // NOTE: Read the note at the top of the file on why we use a generic future
    aos_future_config_t config = {.cb = aos_ws_client_send_text_cb};
    aos_future_t *send_future =
        AOS_FUTURE_ALLOC_T(aos_ws_client_send_text)(&config, data, 0);
